...
const props = defineProps<{
    ...
    filter: { field: string; label?: string; scopes?: Record<string, string>; class?: string } & Record<string, any>;
}>();
...
<template>
    <select :name="filter.field" :class="selectClass" :value="(modelValue as any)"
        ...
        <option value="">-- {{ filter.label || 'Scope' }} --</option>
        <option v-for="(label, key) in (filter.scopes || {})" :key="key" :value="key">{{ label }}</option>
    </select>
</template>
